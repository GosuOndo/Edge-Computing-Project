...
def test_startup_security_sync_detects_missing_and_wrong_bottles_immediately():
    record_1 = {
        "medicine_id": "M001",
        "medicine_name": "Aspirin 100mg",
        "station_id": "station_1",
    ...
    system.database = FakeDatabase(
        records_by_tag={"WRONG2": wrong_record_2},
        records_by_station={"station_1": record_1, "station_2": record_2},
    )
    system.tag_runtime_service = FakeTagRuntimeService(
        latest_by_station={
    ...
    system._bootstrap_registered_station_security_state()
    system._run_startup_station_security_sync(scan_timeout_seconds=0.0)

    assert system.secured_medications["station_1"]["early_alert_sent"] is True
    assert system.secured_medications["station_1"]["present"] is False
    assert system.secured_medications["station_2"]["wrong_bottle_on_station"] is True
    assert system.display.error_calls[-1] == (
        "Station 1 missing bottle | Station 2 wrong bottle"
    )


def test_startup_security_sync_detects_both_wrong_bottles():
    record_1 = {
        "medicine_id": "M001",
        "medicine_name": "Aspirin 100mg",
        "station_id": "station_1",
    ...
    system.state_machine = FakeStateMachine(SystemState.IDLE)
    system.database = FakeDatabase(
        records_by_tag={"WRONG1": wrong_record_1, "WRONG2": wrong_record_2},
        records_by_station={"station_1": record_1, "station_2": record_2},
    )
    system.tag_runtime_service = FakeTagRuntimeService(
        latest_by_station={
